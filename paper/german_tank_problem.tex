...
Goodman \cite{goodman1952serial} also derives a frequentist, two-sided $1-\alpha$ confidence interval for $n$: $m^{(k)}\leq n \leq x$, where $x$ is the largest integer satisfying $(m^{(k)}-1)_{k} /(x)_k \geq \alpha$ with notation defined in eqn.~\ref{eq:falling_factorial}.
